...
from vlm4cluster.methods.ensc.core import run_ensc_pipeline
from vlm4cluster.methods.registry import register_method


@register_method
class EnSCMethod(ClusteringMethod):
    name = "ensc"
    description = "Elastic Net Subspace Clustering with the oracle-based active support solver on OpenCLIP image features."
    category = "image_only"
    ...
    def plan_steps(self, params: dict[str, Any]) -> list[str]:
        ...
        return [
            ...
            image_step,
            "compute the EnSC self-representation matrix with the official elastic-net solver",
    ...
    def run(self, inputs: MethodInputs, params: dict[str, Any]) -> MethodResult:
        outputs = run_ensc_pipeline(inputs, params)
        return MethodResult(
